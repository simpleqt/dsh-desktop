...
The Windows payload uses Electron Builder's `store` compression mode. This produces a larger download but avoids compressed-payload decoding during installation, prioritizing installation speed while retaining the established NSIS path (`useZip=false`).
